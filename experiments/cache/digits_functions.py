...
class Task:
    def __init__(self, pref, X, y, test_X, test_y):
        self.pref = pref
        self.X = X
        self.y = y
        self.test_X = test_X
...
def create_mtl_datasets(X, y, nTasks=1000, taskSize=100, testSize=100): 
    ...
    for i in range(0, nTasks):
        ...
        tX = X[perm[t_ind[:split]], :] # take first taskSize/2 true examples from shuffled dataset
        fX = X[perm[f_ind[:split]], :] # take first taskSize/2 false examples from shuffled dataset
        newX = np.concatenate((tX, fX), axis = 0)
        newy = np.concatenate((np.ones(split), np.zeros(split)), axis = 0)
        ...
        test_tX = X[perm[t_ind[split:(split + test_split)]], :]
        test_fX = X[perm[f_ind[split:(split + test_split)]], :]
        test_newX = np.concatenate((test_tX, test_fX), axis = 0)
        test_newy = np.concatenate((np.ones(test_split), np.zeros(test_split)), axis = 0)
        ...
        task = Task(prefDigit, newX, newy, test_newX, test_newy)
        tasks += [task]
    return tasks
...
def generate_data(tasks):
    ...
    for sid, t in enumerate(tasks):
        ...
        for i in range(len(t.test_y)):
            ...
            test_ss.append(sid)
    print 'finish generate data for LgSegment model!'
    return (xs,ys,ss,test_xs,test_ys,test_ss)
