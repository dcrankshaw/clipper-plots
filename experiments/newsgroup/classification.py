    ...
    def train_ws(self):
        ...
        for s in self.segments:
            ...
            if self.strategy == 'train-all':
                ...
                new_ws.fit(transformed_xs, segment.ys)
...
class UserDefineModel:
    def __init__(self, xs, ys, digitys, ss, strategy, q=20, k=10):
    ...
    def train_ws(self,p=20):
        ...
        for s in self.segments:
            ...
            if self.strategy == 'train-all':
                #print 'strategy is ', self.strategy
                xs = transformed_xs[0:p,:]
                ys = [segment.ys[i] for i in range(0,p)]
                #print 'size of xs is ', xs.shape[0]
                new_ws = lm.LogisticRegression(fit_intercept=False)
                new_ws.fit(xs,ys)
                segment.model = new_ws
                segment.ws = new_ws.coef_[0]
                ...
                #print 'strategy is ', self.strategy
                new_ws = lm.LogisticRegression(fit_intercept=False)
                xs = transformed_xs[p-self.points:p,:]
                ys = [segment.ys[i] for i in range(p-self.points,p)]
                #print 'The size of xs is ', xs.shape[0]
                ...
                ys = np.array([segment.ys[i] for i in range(p-self.points,p)]).reshape((self.points,1))
                step_size = 0.001
                delta = 1.0/p*np.dot(xs.transpose(),(ys- np.dot(xs,ws.reshape((self.k,1)))))
                #ws = ws + step_size*delta.reshape((1,self.k))
                model = segment.model
